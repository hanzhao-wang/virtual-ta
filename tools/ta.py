...
def next_question(args: argparse.Namespace) -> int:
    ...
    if args.out:
        command.extend(["--out", args.out])
    return run_python_script("tools/interactive_practice.py", command)
...
def grade_answer(args: argparse.Namespace) -> int:
    ...
    if args.answer:
        command.extend(["--answer", args.answer])
    if args.answer_file:
        command.extend(["--answer-file", args.answer_file])
    if args.profile:
        command.extend(["--profile", args.profile])
    if args.out:
        command.extend(["--out", args.out])
...
def build_parser() -> argparse.ArgumentParser:
    ...
    next_question_parser = subparsers.add_parser("next-question", help="Internal: generate one interactive question for Codex chat.")
    next_question_parser.add_argument("topic", help="Topic or retry request.")
    next_question_parser.add_argument("--difficulty", default="mixed", help="Difficulty level.")
    next_question_parser.add_argument("--profile", help="Student profile name.")
    next_question_parser.add_argument("--focus", help="Optional adaptation instruction.")
    next_question_parser.add_argument("--top-k", type=int, help="Retrieval chunks to use.")
    next_question_parser.add_argument("--out", help="Output JSON path.")
    next_question_parser.set_defaults(func=next_question)

    grade_answer_parser = subparsers.add_parser("grade-answer", help="Internal: grade a student answer and update memory.")
    grade_answer_parser.add_argument("--question-json", required=True, help="Question JSON generated by next-question.")
    grade_answer_parser.add_argument("--answer", help="Student answer text.")
    grade_answer_parser.add_argument("--answer-file", help="Path to uploaded answer file.")
    grade_answer_parser.add_argument("--profile", help="Student profile name.")
    grade_answer_parser.add_argument("--out", help="Output JSON path.")
    grade_answer_parser.set_defaults(func=grade_answer)
